...
EntityPlayer = ig.Box2DEntity.extend({
	...
	lean: function(direction) {
		if (direction = 'right') {

			if(this.turnAngle < 40) {
				this.rotateTo(this.body.getPosition().x = this.body.getPosition().x + 1);	
			}
		} else {
			if(this.turnAngle > -40) {
				this.rotateTo(this.body.getPosition().x = this.body.getPosition().x - 1);	
	...
	update: function() {
		...
		// power always on
		this.body.ApplyForce( new b2.Vec2(0,-100), this.body.GetPosition() );

		// move left or right
		if( ig.input.state('left') ) {
			this.body.ApplyForce( new b2.Vec2(-200,0), this.body.GetPosition() );
			this.shotDirection = 'left';
			this.lean(this.shotDirection);
		...
		if (this.turnAngle > 360 || this.turnAngle < -360) {
			this.turnAngle = 0;
		}
		...
		// this.body.SetXForm(this.body.GetPosition(), 20);
